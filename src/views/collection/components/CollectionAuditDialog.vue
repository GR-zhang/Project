<template>
  ...
  >
    ...
    > 
      ...
      <!-- 已拒绝显示 -->
      <el-form-item v-if="rowData.auditState == 2" label="拒绝理由" prop="refuseReasons">
        <el-input
          type="textarea"
...
<script>
import { collectionAuditMap, generateOptionsByMap } from '@/config'

export default {
  name: 'CollectionAuditDialog',
  ...
  data() {
    return {
      ...
      options_audit: generateOptionsByMap(collectionAuditMap),
      rules: {
    ...
    _handleSubmit() {
      this.$refs.dialogFormRef.validate(async valid => {
        if(valid) {
          ...
          try{
            await this.$api.audit_one_collection({
              colId: this.rowData.colId,
              auditStatus: this.rowData.auditState,
              auditReason: this.rowData.auditState == 2 ? this.rowData.refuseReasons : undefined
            })
