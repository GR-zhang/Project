  ...
  <el-tabs v-model="activeLang" type="border-card" class="multi-lang-tabs mb_10">
    ...
    >
      <el-form 
        ...
        :model="detailInfo.seriesLangPOS_convert[item.value]" 
        :rules="rules_multiLang" 
        label-width="100px" label-position="left"
      >
        <el-form-item label="系列名称" prop="seriesName">
          <el-input 
            v-model="detailInfo.seriesLangPOS_convert[item.value].seriesName" placeholder="请输入系列名称" clearable />
        </el-form-item>
        <el-form-item label="系列简介" prop="description">
          <el-input
            ...
            placeholder="请输入系列简介"
            v-model="detailInfo.seriesLangPOS_convert[item.value].description" />
        </el-form-item>
      </el-form>
    </el-tab-pane>
  </el-tabs>
...
export default {
  ...
  data() {
    return {
      ...
      rules_multiLang: {
        seriesName: [{required: true, message: '系列名称不能为空', trigger: 'blur'}],
        description: [{required: true, message: '系列简介不能为空', trigger: 'blur'}]
      }
    }
  },
    ...
    async formValidate() {
      try {
        ...
        let containNull = this.langList.some(item => {
          let oneItem =  this.detailInfo.seriesLangPOS_convert[item.value];
          return !oneItem.seriesName || !oneItem.description;
        })
        if(containNull) {
