...
import { logTypeMap_user, generateOptionsByMap } from '@/config'
    ...
    return {
      ...
      queryFormItems: [
        {label: '用户名', prop: 'userName'},
        {
          ...
          options: generateOptionsByMap(logTypeMap_user)
        },
        {
          label: '操作时间',
          prop: 'datetimeRange',
  ...
  methods: {
    ...
    async _getList() {
      tools.setData(this, async () => {
        let res = await this.$api.get_userLog_list({
          params: this.pageParams,
          data: this.queryParams,
        });
        res.data.rows = (res.data.rows || []).map(item => ({
          ...item,
          'logTypeTag': logTypeMap_user.get(item.logType)
        }))
        return res;
      })
    },
