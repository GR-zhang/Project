<template>
  ...
  >
    ...
    > 
      ...
      <!-- 已通过显示 -->
      <el-form-item v-if="rowData.auditState == 2" label="成交手续费" prop="fee">
        <gb-input-number v-model="rowData.fee" unit="%" />
      </el-form-item>
      <!-- 已拒绝显示 -->
  ...
  data() {
    return {
      ...
      rules: {
        ...
        fee: [{required: true, message: '成交手续费不能为空', trigger: 'change'}],
    ...
    _handleSubmit() {
      this.$refs.dialogFormRef.validate(async valid => {
        if(valid) {
          ...
          try{
            await this.$api.audit_one_auction({
              activityId: this.rowData.activityId,
              auditStatus: this.rowData.auditState,
              feeRatio: this.rowData.auditState == 2 ? precisionCalc.div(this.rowData.fee || 0, 100)  : undefined,
              auditReason: this.rowData.auditState == 3 ? this.rowData.refuseReasons : undefined
            })
